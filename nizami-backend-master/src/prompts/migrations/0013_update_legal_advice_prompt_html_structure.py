# Add mandatory HTML structure rules to the answer:
# 1. Opening summary paragraph with key facts and exact penalties upfront
# 2. Structured sections using headings and lists
# 3. Comparison table (HTML <table>) when question involves comparing two legal concepts
# 4. Numbered examples section when relevant
...
1. **Opening summary paragraph** (`<p>`): Start with 1–3 sentences summarizing the core answer.
   - Include the most important facts, exact penalties, and key distinctions upfront.
...
2. **Sections with headings** (`<h3>` or `<strong>`): Divide the answer into clear sections.
   - Use `<h3>` for major sections (e.g. أركان الجريمة, العقوبات, الفرق بين ...).

3. **Bullet lists** (`<ul><li>`): Use for listing elements, conditions, or requirements.

4. **Comparison table** (`<table>`): **REQUIRED** when the question asks to compare two or more legal concepts (e.g. خيانة الأمانة vs السرقة).
   - Structure: `<table><thead><tr><th>وجه المقارنة</th><th>المفهوم الأول</th><th>المفهوم الثاني</th></tr></thead><tbody>...</tbody></table>`
   - Each row must cover a distinct dimension of comparison (e.g. طريقة الحيازة، الركن المادي، العقوبة، صور شائعة).

5. **Numbered examples** (`<ol><li>`): Use when listing case examples or common scenarios.

6. **Notes or exceptions** (`<p><strong>ملاحظة:</strong> ...</p>`): Use for important caveats (e.g. aggravated penalties when fraud is involved).

**Bad example (FORBIDDEN):**
`<p>خيانة الأمانة تختلف عن السرقة في طبيعتها وأركانها...</p>`  ← vague, no numbers, no structure

**Good example (REQUIRED STYLE):**
```
<p>خيانة الأمانة في النظام السعودي هي استيلاء الجاني بسوء نية على مال سُلِّم إليه بناءً على عقد، ويعاقب عليها بالسجن حتى 5 سنوات [1] وغرامة تصل إلى 3 ملايين ريال [1].</p>
<h3>أركان الجريمة</h3>
<ul>
  ...
  <li>القصد الجنائي: اتجاه الإرادة إلى تملك المال [1]</li>
</ul>
<h3>العقوبات (المادة الثانية)</h3>
<ul>
  <li>السجن مدة لا تتجاوز 5 سنوات [1]</li>
  <li>غرامة مالية لا تزيد على 3 ملايين ريال [1]، أو بإحدى هاتين العقوبتين</li>
</ul>
<h3>الفرق بين خيانة الأمانة والسرقة</h3>
<table>
  <thead><tr><th>وجه المقارنة</th><th>خيانة الأمانة</th><th>السرقة</th></tr></thead>
  <tbody>
    <tr><td>حيازة المال</td><td>مشروعة (سُلِّم له المال)</td><td>غير مشروعة (اختلاس خفية)</td></tr>
    <tr><td>الركن المادي</td><td>التسليم المسبق + التصرف بسوء نية</td><td>الاختلاس (الأخذ خفية)</td></tr>
    <tr><td>العقوبة</td><td>سجن حتى 5 سنوات وغرامة [1]</td><td>سجن (تعزير)</td></tr>
  </tbody>
</table>
<h3>صور شائعة</h3>
<ol>
...
<p><strong>ملاحظة:</strong> إذا تضمنت الجريمة وسائل احتيال، قد تصل العقوبة إلى السجن 7 سنوات وغرامة 5 ملايين ريال [1].</p>
```
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("prompts", "0012_update_legal_advice_prompt_full_references"),
    ]
