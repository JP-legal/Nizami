...
export class CountryPickerComponent implements ControlValueAccessor , OnInit {
  ...
  showAvailableJurisdictionForAllUsers = input<boolean>(false);
  ...
  ngOnInit() {
    if(this.showAvailableJurisdictionForAllUsers()){
      this.allowedCountryCodes = this.jurisdiction.jurisdictionsCodes
    } else {
      this.allowedCountryCodes = this.authService.allowedCountries;
    }
  }
